  ...
  <div>
    ...
    <form @submit.prevent="handleSubmit">
      <div v-for="control in formData.controls" :key="control.id">
        ...
        <input v-else-if="control.control === 'TEXT'" :id="control.id" :placeholder="control.caption"
          v-model="formValues[control.id]" :required="control.required === 'true'" class="form-control"
          :tabindex="control.tabIndex" @input="updateValue(control.id, $event.target.value)" />
        <select v-else-if="control.control === 'SELECT'" :id="control.id" v-model="formValues[control.id]"
          :required="control.required === 'true'" class="form-control" :tabindex="control.tabIndex"
        ...
        <div v-else-if="control.control === 'CHECKBOX'" class="form-check">
          <input type="checkbox" :id="control.id" v-model="formValues[control.id]"
            :required="control.required === 'true'" @change="updateValue(control.id, $event.target.checked)"
            class="form-check-input" />
          <label :for="control.id" class="form-check-label">
            {{ control.caption }}
          </label>
        </div>
        <button v-else-if="control.control === 'BUTTON'" type="button" @click="handleButtonClick(control.id)"
          class="form-control">
          {{ control.caption }}
        </button>

        <div v-if="errorMessages.length && control.required === 'true' && !formValues[control.id]"
          class="error-message">
